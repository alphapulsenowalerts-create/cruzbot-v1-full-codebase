...
def test_atr_scaled_notional_inverse():
    ...
    n1 = atr_scaled_notional(1.0, 100.0, base_notional=50, atr_ref_pct=0.01)
    assert abs(n1 - 50.0) < 1e-9
    # 2x ATR → half size
    n2 = atr_scaled_notional(2.0, 100.0, base_notional=50, atr_ref_pct=0.01)
    assert abs(n2 - 25.0) < 1e-9
    # tiny ATR → clipped to max 50
    n3 = atr_scaled_notional(0.1, 100.0, base_notional=50, atr_ref_pct=0.01)
    assert n3 == 50.0
    # huge ATR → clipped to min 10
    n4 = atr_scaled_notional(20.0, 100.0, base_notional=50, atr_ref_pct=0.01, min_notional=10)
    assert n4 == 10.0


def test_atr_sizing_never_exceeds_50_hard_cap():
    settings = Settings(
    ...
    qty, _, _ = rm.size_position(200.0, 100.0, 99.0, atr=0.05)
    assert qty * 100.0 <= 50.0 + 1e-6
    # High ATR shrinks below 50
    qty2, _, _ = rm.size_position(200.0, 100.0, 99.0, atr=4.0)
    assert qty2 * 100.0 <= 50.0 + 1e-6
