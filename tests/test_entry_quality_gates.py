...
def test_post_stop_cooldown(tmp_path: Path):
    ...
    store = BehavioralStateStore(db, post_stop_cooldown_min=20)
    now = datetime(2026, 9, 18, 14, 0, tzinfo=timezone.utc)
    assert store.in_post_stop_cooldown("SOL-USD", now=now) is False
    store.record_post_stop_cooldown("SOL-USD", now=now, minutes=20)
    assert store.in_post_stop_cooldown("SOL-USD", now=now) is True
    assert store.post_stop_cooldown_remaining("SOL-USD", now=now) > 0
...
def test_max_concurrent_positions_blocks_buy():
    ...
    account = AccountState(equity=200.0, cash=100.0, buying_power=100.0)
    decision = Decision(
        action=Action.BUY,
        symbol="ETH-USD",
        confidence=80,
