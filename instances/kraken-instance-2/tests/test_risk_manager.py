"""Risk manager: allowlist, $100/$1000 caps, fractional sizing, circuit breaker."""
...
def test_max_notional_per_trade_100():
    rm = RiskManager(_settings())
    account = AccountState(equity=1600.0, cash=1600.0, buying_power=1600.0)
    decision = Decision(
        action=Action.BUY,
    ...
    assert verdict.sized_qty * 1.2941 <= 100.0 + 1e-6


def test_max_total_exposure_1000():
    rm = RiskManager(_settings())
    account = AccountState(equity=1600.0, cash=1600.0, buying_power=1600.0)
    decision = Decision(
        action=Action.BUY,
    ...
    verdict = rm.evaluate(
        decision, account, entry_price=76284.0, atr=762.84, open_exposure_usd=1000.0
    )
    assert verdict.approved is False
    assert "exposure" in verdict.reason.lower()


def test_fractional_btc_sizing_under_100():
    rm = RiskManager(_settings())
    account = AccountState(equity=1600.0, cash=1600.0, buying_power=1600.0)
    decision = Decision(
        action=Action.BUY,
    ...
    assert verdict.sized_qty * 76284.0 <= 100.0 + 1e-6
    # ~0.001311 BTC at $100 cap
    assert abs(verdict.sized_qty - 100.0 / 76284.0) < 1e-6 or verdict.sized_qty <= 100.0 / 76284.0
...
def test_approved_trade_within_limits():
    ...
    assert verdict.approved is True
    assert verdict.sized_qty > 0
    assert verdict.sized_qty * 150.0 <= 100.0 + 1e-6
