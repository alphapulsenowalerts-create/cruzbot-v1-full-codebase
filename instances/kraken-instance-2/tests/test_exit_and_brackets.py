...
def test_size_position_min_notional_returns_tuple():
    ...
    rm = RiskManager(_settings(MIN_NOTIONAL_USD=10.0, MAX_NOTIONAL_PER_TRADE_USD=100))
...
async def test_paper_book_persist_reload(tmp_path):
    ...
    settings = _settings(PAPER_BOOK_PATH=str(book), ACCOUNT_EQUITY=1600.0)
...
async def test_ledger_skips_rejects(tmp_path, monkeypatch):
    ...
    settings = _settings(ACCOUNT_EQUITY=1600.0, PAPER_BOOK_PATH=str(tmp_path / "book.json"))
    broker = CoinbaseBroker(settings)
    await broker.connect()
    try:
...
def test_long_inverted_tp_never_classifies_as_tp():
    from main import TradingApp

    app = TradingApp(_settings(), once=True)
    pos = Position(
        symbol="ETH-USD", qty=1.0, avg_entry_price=100.0,
        stop_loss=90.0, take_profit=90.0, opened_at=utcnow(),
    )
    decision = app._check_hard_brackets("ETH-USD", pos, mark=95.0)
    assert decision is None or decision.reasoning not in (
        "TP1", "TP2", "ATR take-profit"
    )


def test_fee_shield_holds_breakeven_tp_until_net_positive():
    from main import TradingApp

    app = TradingApp(_settings(MAKER_FEE_RATE=0.005), once=True)
    pos = Position(
        symbol="ETH-USD", qty=1.0, avg_entry_price=100.0,
        stop_loss=90.0, take_profit=100.1, opened_at=utcnow(),
    )
    held = app._check_hard_brackets("ETH-USD", pos, mark=100.1)
    assert held is None or held.reasoning not in (
        "TP1", "TP2", "ATR take-profit"
    )
    profitable = app._check_hard_brackets("ETH-USD", pos, mark=101.1)
    assert profitable is not None
    assert profitable.reasoning == "ATR take-profit"
