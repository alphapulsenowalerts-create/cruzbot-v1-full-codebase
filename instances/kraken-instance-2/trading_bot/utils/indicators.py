...
def compute_indicators(
    ...
    *,
    ...
    Default path is NumPy/pandas fallbacks for sub-10ms in-memory eval.
    Set prefer_numpy=False to try pandas-ta first when installed.
    """
    ...
    used_ta = False
    if (not prefer_numpy) and HAS_PANDAS_TA and ta is not None:
        try:
    ...
    # Vectorized EMA cross labels
    ef = out["ema_fast"].to_numpy(dtype=float, copy=False)
    es = out["ema_slow"].to_numpy(dtype=float, copy=False)
    n = len(out)
    crosses = np.full(n, "none", dtype=object)
    if n >= 2:
        valid = ~(np.isnan(ef) | np.isnan(es))
        valid[1:] = valid[1:] & valid[:-1]
        bull = valid[1:] & (ef[1:] > es[1:]) & (ef[:-1] <= es[:-1])
        bear = valid[1:] & (ef[1:] < es[1:]) & (ef[:-1] >= es[:-1])
        crosses[1:][bull] = "bullish"
        crosses[1:][bear] = "bearish"
...
def atr_scaled_notional(
    ...
    *,
    ...
    Clipped to [min_notional, max_notional]. Hard cap remains max_notional ($100).
