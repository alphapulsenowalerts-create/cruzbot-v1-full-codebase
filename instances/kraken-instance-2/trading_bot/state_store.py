...
class BehavioralStateStore:
    ...
    def get_utc_day_realized_pnl(self, now: Optional[datetime] = None) -> float:
        """Sum trade_memory.pnl for rows whose ts falls on the current UTC calendar day."""
        now = now or utcnow()
        ...
        start_s, end_s = day_start.isoformat(), day_end.isoformat()
        with self._conn() as conn:
            row = conn.execute(
                """
        ...
        try:
            return float(row["day_pnl"] if row is not None else 0.0)
