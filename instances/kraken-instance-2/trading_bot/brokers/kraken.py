...
(``XXBT``, ``ZUSD``). Newer allowlist assets (SOL, LINK, AVAX, SUI, ADA, DOGE, DOT, ATOM, UNI, NEAR; LTC uses XLTC)
...
_WS_URL_DEFAULT = "wss://ws.kraken.com/v2"
...
_PAIR_ROWS: Tuple[Tuple[str, str, str, str, str, str], ...] = (
    ...
    ("DOGE-USD", "DOGEUSD", "DOGE/USD", "DOGEUSD", "DOGE", "ZUSD"),
    ("DOT-USD", "DOTUSD", "DOT/USD", "DOTUSD", "DOT", "ZUSD"),
    ("ATOM-USD", "ATOMUSD", "ATOM/USD", "ATOMUSD", "ATOM", "ZUSD"),
    ("LTC-USD", "LTCUSD", "LTC/USD", "XLTCZUSD", "XLTC", "ZUSD"),
    ("UNI-USD", "UNIUSD", "UNI/USD", "UNIUSD", "UNI", "ZUSD"),
    ("NEAR-USD", "NEARUSD", "NEAR/USD", "NEARUSD", "NEAR", "ZUSD"),
)
...
def to_standard_symbol(symbol: str) -> str:
    ...
    known_bases = (
        "BTC", "XBT", "ETH", "SOL", "XRP", "LINK", "AVAX", "SUI", "ADA",
        "DOGE", "DOT", "ATOM", "LTC", "UNI", "NEAR",
    )
    if compact in known_bases or compact in ("XXBT", "XETH", "XXRP", "XLTC"):
...
def resolve_kraken_pair(symbol: str) -> KrakenPair:
    ...
    alt_base = {"BTC": "XXBT", "ETH": "XETH", "XRP": "XXRP", "LTC": "XLTC"}.get(base, base)
...
def to_kraken_ws_v2_symbol(symbol: str) -> str:
    """WS v2 symbol: ``BTC/USD`` (BTC not XBT). Preserve xStock suffix case (``AAPLx/USD``)."""
    raw = (symbol or "").strip().replace("_", "-")
    if "/" in raw and "-" not in raw:
        raw = raw.replace("/", "-", 1)
    standard = to_standard_symbol(raw)
    base, quote = standard.split("-", 1) if "-" in standard else (standard, "USD")
    # Recover trailing 'x' for tokenized equities if uppercasing ate it
    src_base = raw.split("-", 1)[0] if "-" in raw else raw
    if src_base.lower().endswith("x") and not base.endswith("x") and not base.endswith("X"):
        base = base[:-1] + "x" if len(base) > 1 else base
    elif src_base.endswith("x") and base.endswith("X"):
        base = base[:-1] + "x"
    if base.upper() in ("XBT", "XXBT", "BTC"):
        base = "BTC"
    return f"{base}/{quote}"


def from_kraken_ws_v2_symbol(pair: str) -> str:
    """``BTC/USD`` / ``AAPLx/USD`` → standard ``BTC-USD``."""
    return to_standard_symbol(str(pair).replace("/", "-"))


def ws_v2_subscribe_ticker(symbols: List[str]) -> Dict[str, Any]:
    """Kraken WS v2 ticker subscribe (thin MVP — prices only)."""
    return {
        "method": "subscribe",
        "params": {
            "channel": "ticker",
            "symbol": [to_kraken_ws_v2_symbol(s) for s in symbols],
        },
    }
...
class KrakenBroker(BrokerAdapter):
    ...
    def __init__(self, settings: Settings) -> None:
        ...
        self._wallet_b4: Optional[float] = None  # paper starting bankroll (pre W/L)
        ...
        self._symbol_tick_mono: Dict[str, float] = {}
    ...
    def _load_paper_book(self) -> None:
        ...
        try:
            ...
            wb4 = raw.get("wallet_b4")
            if wb4 is not None:
                self._wallet_b4 = float(wb4)
            elif self._wallet_b4 is None:
                # Seed once: original funded bankroll (prefer ACCOUNT_EQUITY)
                try:
                    seed = float(getattr(self.settings, "account_equity", 0) or 0)
                except Exception:
                    seed = 0.0
                self._wallet_b4 = seed if seed > 0 else float(cash)
    ...
    def _save_paper_book(self) -> None:
        ...
        try:
            ...
            if self._wallet_b4 is None:
                try:
                    seed = float(getattr(self.settings, "account_equity", 0) or 0)
                except Exception:
                    seed = 0.0
                self._wallet_b4 = seed if seed > 0 else float(self._paper_cash)
            payload: Dict[str, Any] = {
                "cash": self._paper_cash,
                "equity": self._paper_equity,
                "day_start_equity": self._day_start_equity,
                "wallet_b4": self._wallet_b4,
    ...
    def update_position_brackets(
        ...
        *,
        ...
        if mark_price is not None and mark_price > 0:
            ...
            pos.market_value = (-pos.qty * mark_price) if str(getattr(pos, "side", "long")).lower() == "short" else pos.qty * mark_price
            pos.unrealized_pl = ((pos.avg_entry_price - mark_price) if str(getattr(pos, "side", "long")).lower() == "short" else (mark_price - pos.avg_entry_price)) * pos.qty
            if update_trail_hwm and (force_trail or not sweet):
                if str(getattr(pos, "side", "long")).lower() == "short":
                    if pos.trail_high_water is None or mark_price < pos.trail_high_water:
                        pos.trail_high_water = mark_price
                elif pos.trail_high_water is None or mark_price > pos.trail_high_water:
                    pos.trail_high_water = mark_price
    ...
    def paper_wallet_b4(self) -> float:
        """Paper starting bankroll (pre W/L). Falls back to ACCOUNT_EQUITY."""
        if self._wallet_b4 is not None and float(self._wallet_b4) > 0:
            return float(self._wallet_b4)
        try:
            seed = float(getattr(self.settings, "account_equity", 0) or 0)
        except Exception:
            seed = 0.0
        if seed > 0:
            return seed
        return float(self._paper_cash or 0.0)
    ...
    def reset_paper_book(self, cash: Optional[float] = None) -> None:
        ...
        self._wallet_b4 = float(self._paper_cash)  # new starting bankroll
        self._save_paper_book()
        logger.warning("Paper book RESET to cash=%.2f flat (Wallet B4=%.2f)", self._paper_cash, self._wallet_b4)

    def close_paper_position_breakeven(self, symbol: str) -> Optional[dict]:
        """Force-close a paper position at avg entry ($0 PnL). Returns summary or None."""
        product = to_standard_symbol(symbol)
        pos = self._paper_positions.get(product)
        if pos is None:
            # try loose match
            for k, v in list(self._paper_positions.items()):
                if k.upper().replace("_", "-") == product.upper().replace("_", "-"):
                    product, pos = k, v
                    break
        if pos is None:
            return None
        ...
        if qty <= 1e-12 or entry <= 0:
            self._paper_positions.pop(product, None)
            self._save_paper_book()
            return {"symbol": product, "qty": 0.0, "entry": entry, "proceeds": 0.0}
        proceeds = qty * entry
        if side == "short":
            # covering short at entry: cash was credited on open; debit entry*qty
            self._paper_cash -= proceeds
        else:
            self._paper_cash += proceeds
        self._paper_positions.pop(product, None)
        pos_value = sum(float(p.market_value) for p in self._paper_positions.values())
        self._paper_equity = self._paper_cash + pos_value
        self._save_paper_book()
        logger.warning(
            "Paper BE close %s qty=%.6g entry=%.6g proceeds=%.2f cash=%.2f",
            product, qty, entry, proceeds, self._paper_cash,
        )
        return {
            "symbol": product,
            "qty": qty,
    ...
    def close_all_paper_xstocks_breakeven(self) -> list:
        """Close every tokenized x-USD paper position at breakeven."""
        from trading_bot.market_regime import is_tokenized_symbol
        closed = []
        for sym in list(self._paper_positions.keys()):
            if is_tokenized_symbol(sym):
                row = self.close_paper_position_breakeven(sym)
                if row:
                    closed.append(row)
        return closed
    ...
    def _touch_symbol_tick(self, symbol: str) -> None:
        product = to_standard_symbol(symbol)
        now = time.monotonic()
        self._symbol_tick_mono[product] = now
        self._last_tick_mono = now

    def symbol_tick_age_seconds(self, symbol: str) -> Optional[float]:
        product = to_standard_symbol(symbol)
        mono = self._symbol_tick_mono.get(product)
        if mono is None:
            return None
    ...
    async def force_reconnect_market_data(self, *, reason: str = "stale") -> None:
        async with self._reconnect_lock:
            ...
            try:
                ...
                # Paper/local only: clear unfilled resting orders so a WS drop
                # cannot leave orphan working orders across reconnect.
                if self.settings.paper_trading_mode:
                    try:
                        n = await self.cancel_all_orders()
                        if n:
                            logger.info(
                                "Reconnect: cancelled %s pending paper/local open orders",
                                n,
                            )
                    except Exception as exc:
                        logger.warning("Reconnect cancel_all_orders: %s", exc)
    ...
    async def fetch_live_trades_history(self, *, days: int = 7) -> List[Dict[str, Any]]:
        """Kraken TradesHistory → list of {symbol, entry, exit, pnl, ts, reason} for digest.

        Never used for paper. Approximates round-trip PnL from consecutive buy/sell pairs
        when cost fields are present; otherwise fee-adjusted sell proceeds only.
        """
        import time as _time
        days = max(1, min(int(days), 90))
        start = _time.time() - days * 86400.0
        out: List[Dict[str, Any]] = []
        try:
            self._require_credentials()
        except Exception as exc:
            logger.warning("live trades history: no credentials (%s)", exc)
            return out
        try:
            result = await self._private(
                "TradesHistory",
                {"start": str(int(start)), "ofs": "0"},
            ) or {}
        except Exception as exc:
            logger.warning("TradesHistory failed: %s", exc)
            return out
        trades = (result.get("trades") or {}) if isinstance(result, dict) else {}
        # Group by pair chronologically; match buy→sell for net
        by_pair: Dict[str, List[Dict[str, Any]]] = {}
        for tid, tr in trades.items():
            try:
                pair = from_kraken_pair(str(tr.get("pair") or ""))
                typ = str(tr.get("type") or "").lower()
                px = float(tr.get("price") or 0)
                vol = float(tr.get("vol") or 0)
                fee = float(tr.get("fee") or 0)
                cost = float(tr.get("cost") or (px * vol))
                ts = float(tr.get("time") or 0)
            except Exception:
                continue
            by_pair.setdefault(pair, []).append(
                {"type": typ, "price": px, "vol": vol, "fee": fee, "cost": cost, "ts": ts, "id": tid}
            )
        for pair, rows in by_pair.items():
            ...
            for r in rows:
                if r["type"] == "buy":
                    open_buys.append(r)
                    continue
                if r["type"] != "sell" or not open_buys:
                    # orphan sell — count fee-only pnl unknown; skip
                    continue
                buy = open_buys.pop(0)
                # PnL ≈ sell cost - buy cost - fees
                pnl = (r["cost"] - r["fee"]) - (buy["cost"] + buy["fee"])
                out.append(
                    {
                        "symbol": pair,
        ...
        if self.settings.paper_trading_mode:
            for sym, pos in list(self._paper_positions.items()):
                ...
                if str(getattr(pos, "side", "long")).lower() == "short":
                    ...
                    pos.unrealized_pl = (pos.avg_entry_price - px) * pos.qty
                else:
                    pos.market_value = pos.qty * px
    ...
    async def get_quote(self, symbol: str) -> Quote:
        product = to_standard_symbol(symbol)
        # Prefer warm in-memory ticker (WS / prior fill) — kill REST scan latency
        cached = self._paper_prices.get(product)
        age = self.symbol_tick_age_seconds(product) if cached else None
        if cached and cached > 0 and age is not None and age < 10.0:
            mid = float(cached)
            spread = mid * 0.0005
            return Quote(
                symbol=product,
        ...
        if self.settings.paper_trading_mode and self._session is None:
            ...
            spread = mid * 0.0005
            self._touch_symbol_tick(product)
            return Quote(
                symbol=product,
                timestamp=utcnow(),
                bid=round(mid - spread / 2, 6),
    ...
    def _simulate_fill(self, order: OrderRequest) -> OrderResult:
        ...
        def reject(message: str) -> OrderResult:
            result = OrderResult(client_order_id=order.client_order_id, broker_order_id=broker_id,
                status=OrderStatus.REJECTED, symbol=product, side=order.side, qty=order.qty,
                message=message, paper=True)
            self._paper_orders[broker_id] = result
            return result

        if order.side == OrderSide.BUY:
            # BUY opens long while flat, or covers a paper short.
            if existing and str(getattr(existing, "side", "long")).lower() == "short":
                cover_qty = min(existing.qty, order.qty)
                cover_cost = px * cover_qty
                cover_fee = _apply_trade_fee(self.settings, cover_cost, is_taker=is_taker)
                if cover_cost + cover_fee > self._paper_cash + 1e-6:
                    return reject("paper: insufficient cash to cover short")
                self._paper_cash -= cover_cost + cover_fee
                existing.qty -= cover_qty
                if existing.qty <= 1e-9:
                    del self._paper_positions[product]
                    existing = None
                else:
                    existing.market_value = -existing.qty * px
                    existing.unrealized_pl = (existing.avg_entry_price - px) * existing.qty
                # MVP does not flip residual cover quantity into a long.
                if cover_qty < order.qty - 1e-9:
                    return reject("paper: buy exceeds short cover quantity")
            else:
                if cost + fee > self._paper_cash + 1e-6:
                    return reject("paper: insufficient cash")
                self._paper_cash -= cost + fee
                if existing:
                    new_qty = existing.qty + order.qty
                    existing.avg_entry_price = (existing.avg_entry_price * existing.qty + cost) / new_qty
                    ...
                    self._paper_positions[product] = Position(
                        symbol=product, qty=order.qty, avg_entry_price=px, market_value=cost,
                        side="long", stop_loss=order.stop_loss, take_profit=order.take_profit,
                        trail_distance=None, trail_high_water=None if sweet else px,
                        opened_at=utcnow(), tp1_done=False, initial_qty=order.qty)
        else:
            # SELL closes a long, or opens/increases a paper short.
            if existing and str(getattr(existing, "side", "long")).lower() == "long":
                if existing.qty < order.qty - 1e-9:
                    return reject("paper: insufficient position")
                self._paper_cash += cost - fee
                ...
                # Spot Kraken cannot short — refuse sell-to-open / add-to-short.
                allow_shorts = bool(getattr(self.settings, "allow_paper_shorts", False))
                if not existing or str(getattr(existing, "side", "long")).lower() != "long":
                    if not allow_shorts:
                        return reject("paper: shorts disabled (spot long-only)")
                if not existing:
                    self._paper_cash += cost - fee
                    self._paper_positions[product] = Position(
                        symbol=product, qty=order.qty, avg_entry_price=px, market_value=-cost,
                        side="short", stop_loss=order.stop_loss, take_profit=order.take_profit,
                        trail_distance=None, trail_high_water=None, opened_at=utcnow(),
                        tp1_done=False, initial_qty=order.qty, entry_reason="paper short")
                elif str(getattr(existing, "side", "long")).lower() == "short":
                    if not allow_shorts:
                        return reject("paper: shorts disabled (spot long-only)")
                    self._paper_cash += cost - fee
                    new_qty = existing.qty + order.qty
        ...
        self._paper_prices[product] = px
        result = OrderResult(client_order_id=order.client_order_id, broker_order_id=broker_id,
            status=OrderStatus.FILLED, symbol=product, side=order.side, qty=order.qty,
            filled_qty=order.qty, avg_fill_price=px,
            message="paper simulated fill (no live Kraken submit)", paper=True)
        self._paper_orders[broker_id] = result
        self._save_paper_book()
        return result
    ...
    async def liquidate_all(self) -> List[OrderResult]:
        if self.settings.paper_trading_mode:
            ...
            for pos in list(await self.get_positions()):
                ...
                if not self.settings.is_allowlisted(_sym):
    ...
    async def stream_quotes(
        ...
        """Public Kraken WS v2 ticker → in-memory ``_paper_prices`` cache.

        Subscribe ``ticker`` on ``wss://ws.kraken.com/v2`` for the runtime
        universe (crypto + xStocks). Scan/get_quote reads memory when warm.
        Falls back to REST poll if ``websockets`` missing.
        """
        try:
            import websockets
        ...
        if "/v2" not in url:
            url = url.rstrip("/") + "/v2"
        # chunk symbols (Kraken caps batch size); 50 per sub is safe
        syms = list(symbols)
        chunks = [syms[i : i + 50] for i in range(0, len(syms), 50)] or [[]]
        backoff = ExponentialBackoff(base=1.0, max_delay=60.0)
        logger.info("Kraken WS v2 ticker connecting url=%s symbols=%d", url, len(syms))
        while self._connected:
            try:
                async with websockets.connect(url, ping_interval=20, ping_timeout=20) as ws:
                    for chunk in chunks:
                        if not chunk:
                            continue
                        await ws.send(json.dumps(ws_v2_subscribe_ticker(chunk)))
                    backoff.reset()
                    logger.info("Kraken WS v2 ticker subscribed symbols=%d", len(syms))
                    async for raw in ws:
                        ...
                        for row in data:
                            ...
                            if bid <= 0 and ask <= 0 and last <= 0:
                                continue
                            product = from_kraken_ws_v2_symbol(str(pair))
                            mid = (
                                (bid + ask) / 2
                                if (bid and ask)
                                else (last or bid or ask)
                            )
                            self._paper_prices[product] = mid
                            self._touch_symbol_tick(product)
                            on_quote(
                ...
                delay = backoff.next_delay()
                logger.warning("Kraken WS v2 quote error (%s); retry in %.1fs", exc, delay)
                await asyncio.sleep(delay)
